...
<?php
	...
	if (isset($_COOKIE["username"])){
		$con = mysqli_connect("localhost", "root", "qiaopengju") or die("Could not connect database:" .mysqli_error());
		mysqli_select_db($con, "photograph");
		for ($i = 0; $i < $file_len; $i++){//循环添加文件
			if (!file_exists("photo_db/" .$_FILES["file"]["name"][$i])){
				$img_src = "photo_db/" . $_COOKIE["username"] . "_" . 
					$_POST["graphy"] . "_" . $_FILES["file"]["name"][$i];
